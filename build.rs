...
fn main() {
    ...
    let Some(package_manager) = KNOWN_PACKAGE_MANAGERS
        ...
        panic!("Could not find a suitable node package manager. Tried {KNOWN_PACKAGE_MANAGERS:#?}")
    };
    ...
    match Command::new(package_manager)
        ...
        .spawn()
        .unwrap_or_else(|_| panic!("Could not run the selected package manager: {package_manager}"))
        .wait()
    {
